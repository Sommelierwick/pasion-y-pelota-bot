...
5. **Persistencia de Partidos en Vivo Pasada la Medianoche (Bug de Desaparición):** Corregimos un bug crítico en la marquesina/widget de partidos de hoy en `front-page.php`. Anteriormente, al llegar la medianoche (cambio de fecha calendario local), los partidos nocturnos en juego desaparecían de inmediato del widget porque su fecha de inicio pertenecía al "día anterior". Modificamos la query en PHP para incluir de forma permanente cualquier partido cuyo estado sea "en vivo" (Status: `vivo`, `tiempo`, `entretiempo`, `1t`, `2t`) independientemente de su fecha de inicio, manteniendo la visibilidad del fixture activo a toda hora.
